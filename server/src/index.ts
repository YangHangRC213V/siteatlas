 ...
 *   SITEATLAS_ROOT         仓库根目录，默认自动向上查找 npm workspaces 根
...
const rootDir = process.env['SITEATLAS_ROOT'] ?? findProjectRoot(import.meta.dirname);
...
const handle = openDb({
  rootDir,
  ...(process.env['SITEATLAS_DB'] !== undefined ? { file: process.env['SITEATLAS_DB'] } : {}),
});
